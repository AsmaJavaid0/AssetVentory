...
class _AssetsScreenState extends State<AssetsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _addAsset() async { await AddQuickAssetSheet.show(context); if (!mounted) return; await _load(); }
  ...
  Future<void> _addCategory() async {
    ...
    final created = await CreateCategorySheet.show(
      ...
      onCreateCategory: (name, emoji) async {
        createdCategoryId = await _categoryRepository.createCategoryIfNotExists(
          ownerId: _ownerId,
          name: name,
          emoji: emoji,
        );
