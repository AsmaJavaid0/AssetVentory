...
class _AddAssetScreenState extends State<AddAssetScreen> {
  ...
  Future<void> _pickDocuments() async {
    try {
      final files = await FilePicker.pickFiles(allowMultiple: true, withData: false);
              ...
              InkWell(
                onTap: () async {
                  final selected = await showModalBottomSheet<String>(
                    ...
                    backgroundColor: Colors.white,
                    shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
                    builder: (pickerCtx) => SafeArea(child: SizedBox(height: 390, child: EmojiPicker(onEmojiSelected: (_, value) => Navigator.pop(pickerCtx, value.emoji)))),
                  );
                ...
                child: Container(
                  ...
                  child: Row(children: [Text(catEmoji, style: const TextStyle(fontSize: 30)), const SizedBox(width: 12), const Expanded(child: Text('Choose any emoji')), const Icon(Icons.chevron_right_rounded)]),
                ),
              ),
            ],
          ),
  ...
  Future<void> _saveAsset() async {
    ...
    try {
      ...
      final asset = LocalAsset(
        id: const Uuid().v4(), ownerId: 'local_user', name: _nameController.text.trim(), categoryId: _selectedCategoryId,
        emoji: _selectedEmoji, location: _locationController.text.trim().isEmpty ? null : _locationController.text.trim(),
        description: _descriptionController.text.trim().isEmpty ? null : _descriptionController.text.trim(), imagePath: _primaryImage?.path,
        createdAt: now, updatedAt: now, qrEnabled: _qrEnabled,
      );
      await _assetRepository.createAsset(asset);
      for (final file in _documents) {
        await _documentRepository.addDocument(assetId: asset.id, sourceFile: file, displayName: file.uri.pathSegments.isEmpty ? 'Document' : file.uri.pathSegments.last);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text('Create Asset', style: GoogleFonts.outfit(fontWeight: FontWeight.w700)), backgroundColor: Colors.white, elevation: 0),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            InkWell(
              ...
              child: Container(
                ...
                child: _primaryImage == null
                    ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.add_a_photo_outlined, size: 38, color: AppColors.primaryPurple), const SizedBox(height: 8), Text('Add Asset Photo', style: GoogleFonts.outfit(fontWeight: FontWeight.w600))]))
                    : ClipRRect(borderRadius: BorderRadius.circular(20), child: Image.file(_primaryImage!, fit: BoxFit.cover)),
              ),
            ),
            ...
            DropdownButtonFormField<String>(
              ...
              items: [..._categories.map((c) => DropdownMenuItem(value: c.id, child: Text('${c.emoji ?? '📂'} ${c.name}'))), const DropdownMenuItem(value: '__create__', child: Text('+ Create New Category'))],
              onChanged: (v) { if (v == '__create__') _showCreateCategoryDialog(); else setState(() => _selectedCategoryId = v); },
            ),
            const SizedBox(height: 14),
            ...
            Container(
              ...
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.inputBorder)),
              child: Column(
