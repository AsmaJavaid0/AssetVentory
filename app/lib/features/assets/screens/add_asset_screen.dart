...
class _AddAssetScreenState extends State<AddAssetScreen> {
  ...
  Future<void> _showCreateCategoryDialog() async {
    String? createdId;
    final created = await CreateCategorySheet.show(context, onCreateCategory: (name, emoji) async {
      createdId = await _categoryRepository.createCategoryIfNotExists(ownerId: 'local_user', name: name, emoji: emoji);
    });
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Form(key: _formKey, child: ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 40), children: [
      ...
      CustomTextField(controller: _nameController, labelText: 'Asset Name *', hintText: 'e.g. Honda Bike', validator: (value) { if (value == null || value.trim().isEmpty) return 'Enter an asset name'; return null; }),
