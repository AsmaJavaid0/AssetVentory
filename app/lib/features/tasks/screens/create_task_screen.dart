...
class CreateTaskScreen extends StatefulWidget {
  ...
  static Future<void> navigateTo(BuildContext context, {LocalAsset? preselectedAsset}) {
    return Navigator.push(context, MaterialPageRoute(builder: (context) => CreateTaskScreen(preselectedAsset: preselectedAsset)));
  }
...
class _CreateTaskScreenState extends State<CreateTaskScreen> {
  ...
  // Always initialize from the phone's current local clock. The notification
  // service converts this local wall-clock value using the device timezone.
  ...
  void initState() {
    ...
    if (defaultPriorityStr == 'low') _priority = TaskPriority.low;
    else if (defaultPriorityStr == 'high') _priority = TaskPriority.high;
    else _priority = TaskPriority.medium;
  ...
  Future<void> _checkFamilyMembership() async {
    ...
    if (user != null) {
      try {
        ...
        if (mounted) setState(() { _userFamily = family; _isLoadingFamily = false; });
      } catch (_) {
  ...
  Future<void> _saveTask() async {
    ...
    try {
      // Construct the scheduled DateTime from the selected calendar date and
      // the selected device-local clock time. Do not convert it to UTC here.
      // TaskNotificationService is responsible for mapping this local value
      // to the device timezone used by Android/iOS alarms.
      final now = DateTime.now();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Create Task', style: GoogleFonts.outfit(fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            Text('Task Type', style: GoogleFonts.outfit(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
            ...
            Wrap(
              ...
              children: TaskType.values.where((t) => t != TaskType.familyTask).map((type) {
                ...
                return ChoiceChip(label: Text('${type.icon} ${type.label}'), selected: isSelected, selectedColor: AppColors.primaryPurple, backgroundColor: Colors.white, labelStyle: GoogleFonts.outfit(color: isSelected ? Colors.white : AppColors.textPrimary, fontWeight: isSelected ? FontWeight.w600 : FontWeight.normal, fontSize: 13), onSelected: (_) => setState(() => _taskType = type));
              }).toList(),
            ),
            const SizedBox(height: 20),
            Text('Associated Asset', style: GoogleFonts.outfit(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
            const SizedBox(height: 6),
            InkWell(
              onTap: () async {
                final asset = await AssetPickerSheet.show(context, selectedAsset: _selectedAsset);
              ...
              child: Container(
                ...
                child: Row(children: [Text(_selectedAsset?.emoji ?? '📦', style: const TextStyle(fontSize: 20)), const SizedBox(width: 12), Expanded(child: Text(_selectedAsset?.name ?? 'No asset selected', style: GoogleFonts.outfit(fontSize: 15, fontWeight: _selectedAsset != null ? FontWeight.w600 : FontWeight.normal, color: _selectedAsset != null ? AppColors.textPrimary : AppColors.textMuted))), const Icon(Icons.chevron_right_rounded, color: AppColors.textMuted)]),
              ),
            ),
            const SizedBox(height: 20),
            Row(children: [
            ...
            if (_reminderEnabled) ...[
              const SizedBox(height: 8),
              DropdownButtonFormField<int>(value: _reminderMinutesBefore, decoration: const InputDecoration(labelText: 'Remind me before'), items: const [5, 10, 15, 30, 60, 120].map((m) => DropdownMenuItem(value: m, child: Text('$m minutes before'))).toList(), onChanged: (v) { if (v != null) setState(() => _reminderMinutesBefore = v); }),
            ],
            const SizedBox(height: 24),
            CustomButton(text: _isSaving ? 'Saving...' : 'Save Task', onPressed: _isSaving ? null : _saveTask),
          ],
        ),
      ),
    );
