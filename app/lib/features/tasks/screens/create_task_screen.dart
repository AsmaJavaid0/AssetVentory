...
class _CreateTaskScreenState extends State<CreateTaskScreen> {
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: Form(key: _formKey, child: ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 40), children: [
      ...
      Wrap(spacing: 8, runSpacing: 8, children: TaskType.values.where((t) => t != TaskType.familyTask).map((type) {
        final selected = type == _taskType;
        return ChoiceChip(label: Text('${type.icon} ${type.label}'), selected: selected, selectedColor: AppColors.primaryPurple, backgroundColor: Colors.white, labelStyle: GoogleFonts.outfit(color: selected ? Colors.white : AppColors.textPrimary, fontWeight: selected ? FontWeight.w600 : FontWeight.normal, fontSize: 13), onSelected: (_) => setState(() { _taskType = type; if (type == TaskType.alarm) _reminderMinutesBefore = 0; }));
      }).toList()),
      const SizedBox(height: 20),
      ...
      const SizedBox(height: 20),
      SwitchListTile.adaptive(contentPadding: EdgeInsets.zero, title: Text('Alarm & Reminder', style: GoogleFonts.outfit(fontWeight: FontWeight.w600)), subtitle: Text('Rings alarm on phone at scheduled time', style: GoogleFonts.outfit(fontSize: 12, color: AppColors.textSecondary)), value: _reminderEnabled, activeTrackColor: AppColors.primaryPurple, onChanged: (v) => setState(() => _reminderEnabled = v)),
      if (_reminderEnabled) ...[
        const SizedBox(height: 8),
        DropdownButtonFormField<int>(key: ValueKey(_reminderMinutesBefore), initialValue: _reminderMinutesBefore, decoration: const InputDecoration(labelText: 'Remind me', prefixIcon: Icon(Icons.notifications_active_outlined)), items: const [DropdownMenuItem(value: 0, child: Text('At time of task (On time)')), DropdownMenuItem(value: 5, child: Text('5 minutes before')), DropdownMenuItem(value: 10, child: Text('10 minutes before')), DropdownMenuItem(value: 15, child: Text('15 minutes before')), DropdownMenuItem(value: 30, child: Text('30 minutes before')), DropdownMenuItem(value: 60, child: Text('1 hour before')), DropdownMenuItem(value: 1440, child: Text('1 day before'))], onChanged: (v) { if (v != null) setState(() => _reminderMinutesBefore = v); }),
      ],
