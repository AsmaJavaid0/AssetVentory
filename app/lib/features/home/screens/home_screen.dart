...
class _HomeScreenState extends State<HomeScreen> {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [Text('My Collection', style: GoogleFonts.outfit(fontSize: 24, color: Colors.white, fontWeight: FontWeight.w700)), const SizedBox(width: 6), const Text('👋', style: TextStyle(fontSize: 20))]),
          ]),
        ...
        Row(children: [Expanded(child: Text('Everything you own,\norganized in one place.', style: GoogleFonts.outfit(fontSize: 14, color: const Color(0xFFB8AED6), height: 1.4))), const SizedBox(width: 76, height: 76, child: AssetLogo(size: 58, showText: false, isDarkBackground: true))]),
      ]),
    ),
  );
  ...
  Widget _buildAddAssetBanner(BuildContext context) => GestureDetector(
    ...
    child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 16), decoration: BoxDecoration(gradient: AppColors.primaryGradient, borderRadius: BorderRadius.circular(22)), child: Row(children: [
      Container(width: 48, height: 48, decoration: BoxDecoration(color: Colors.white.withAlpha(50), shape: BoxShape.circle), child: const Icon(Icons.add_rounded, color: Colors.white, size: 28)), const SizedBox(width: 14),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_assets.isEmpty ? 'Add Your First Asset' : 'Add Asset', style: GoogleFonts.outfit(fontSize: 17, fontWeight: FontWeight.w700, color: Colors.white)), const SizedBox(height: 3), Text(_assets.isEmpty ? 'Start organizing your belongings.' : 'Add another asset to your collection.', style: GoogleFonts.outfit(fontSize: 12, color: Colors.white.withAlpha(220)))])),
      const Icon(Icons.chevron_right_rounded, color: Colors.white, size: 28),
    ])),
  );

  Widget _buildMetrics() => Row(children: [
    _metric(Icons.inventory_2_rounded, '${_assets.length}', 'Assets', () => widget.onTabSelected?.call(1)), const SizedBox(width: 8),
    _metric(Icons.folder_rounded, '${_categories.length}', 'Categories', _openCategories), const SizedBox(width: 8),
    _metric(Icons.task_alt_rounded, '0', 'Tasks', () => widget.onTabSelected?.call(3)), const SizedBox(width: 8),
    _metric(Icons.group_rounded, '—', 'Family', () => widget.onTabSelected?.call(2)),
  ]);

  Widget _metric(IconData icon, String count, String label, VoidCallback onTap) => Expanded(child: GestureDetector(onTap: onTap, child: Container(padding: const EdgeInsets.symmetric(vertical: 13, horizontal: 5), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFEFEBF6))), child: Column(children: [Icon(icon, color: AppColors.primaryPurple, size: 22), const SizedBox(height: 7), Text(count, style: GoogleFonts.outfit(fontSize: 17, fontWeight: FontWeight.w700, color: AppColors.textPrimary)), const SizedBox(height: 2), Text(label, style: GoogleFonts.outfit(fontSize: 9, color: AppColors.textSecondary))]))));
  ...
  Widget _buildRecentAssets() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Recent Assets', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textPrimary)), TextButton(onPressed: () => widget.onTabSelected?.call(1), child: Text('View all', style: GoogleFonts.outfit(color: AppColors.primaryPurple, fontWeight: FontWeight.w600)))]),
  ...
  Widget _buildAssetTile(LocalAsset asset) => GestureDetector(
    ...
    child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(13), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFEFEBF6))), child: Row(children: [
      ClipRRect(borderRadius: BorderRadius.circular(12), child: Container(width: 82, height: 58, color: AppColors.primaryPurple.withAlpha(18), child: asset.imagePath != null && asset.imagePath!.isNotEmpty ? Image.file(File(asset.imagePath!), fit: BoxFit.cover, errorBuilder: (_, _, _) => _emojiFallback(asset)) : _emojiFallback(asset))),
