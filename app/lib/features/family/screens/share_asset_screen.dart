...
class _ShareAssetScreenState extends State<ShareAssetScreen> {
  ...
  Future<void> _continue() async {
    ...
    final result = await ShareAssetPermissionsScreen.navigateTo(
      ...
      asset: asset,
      categoryName:
          asset.categoryId == null ? null : _categories[asset.categoryId],
    );
  ...
  Widget _assetTile(LocalAsset asset) {
    ...
    final category = asset.categoryId == null
        ? null
        : _categories[asset.categoryId];

    // Material is intentionally inside the decorated container so ListTile's
    // ink splash is painted above the container background. This removes the
    // Flutter "ListTile background color or ink splashes may be invisible"
    // assertion without changing the visual design.
    return Container(
      ...
      child: Material(
        ...
        child: ListTile(
          ...
          subtitle: Text(
            already ? 'Already shared with family' : category ?? 'Uncategorized',
          ),
          trailing: already
              ? const Icon(
            ...
            : _error != null
                ...
                : _assets.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (_, index) => _assetTile(_assets[index]),
                      ),
      );
}
