...
class _FamilySharePinScreenState extends State<FamilySharePinScreen> {
  ...
  Future<void> _verify() async {
    ...
    try {
      ...
      if (ok) {
        ...
        _message('Incorrect PIN.', error: true);
      }
    } catch (e) {
  ...
  Future<void> _savePin() async {
    ...
    if (pin != confirm) {
      _message('PINs do not match.', error: true);
      return;
    }
    setState(() => _loading = true);
    try {
      await _repository.setFamilySharePin(familyId: widget.family.id, pin: pin);
      if (!mounted) return;
      _message('Family Share PIN enabled. Enter it once to unlock shared assets.');
      Navigator.pop(context, true);
    } catch (e) {
      if (mounted) _message(ErrorFormatter.format(e), error: true);
    } finally {
  ...
  Future<void> _removePin() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Remove Family Share PIN?'),
        content: const Text('Shared assets will no longer require the family PIN.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Remove PIN')),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final title = widget.unlockOnly ? 'Unlock Shared Assets' : 'Family Share PIN';
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.lock_outline_rounded, color: AppColors.primaryPurple, size: 34),
                      ),
                      ...
                      Text(
                        widget.unlockOnly
                            ? 'Enter the PIN set by the family owner to view and manage shared assets.'
                            : 'Protect your family\'s shared assets with a separate 4–6 digit PIN. The PIN is never stored in plain text.',
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: widget.unlockOnly ? 'Family PIN' : 'New PIN',
                      ...
                      SizedBox(
                        ...
                        height: 52,
                        child: FilledButton(
                          onPressed: widget.unlockOnly ? _verify : _savePin,
                          style: FilledButton.styleFrom(
                            backgroundColor: AppColors.primaryPurple,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                          ),
                          child: Text(widget.unlockOnly ? 'Unlock Shared Assets' : (enabled ? 'Update PIN' : 'Enable PIN')),
                        ),
                      ),
                      if (!widget.unlockOnly && enabled && _isOwner) ...[
                        const SizedBox(height: 10),
                        TextButton.icon(
                          onPressed: _removePin,
                          icon: const Icon(Icons.lock_open_rounded, color: AppColors.error),
